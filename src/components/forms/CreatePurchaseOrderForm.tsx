    ...
    if (Array.isArray(watchedLineItems)) {
      watchedLineItems.forEach((item, index) => {
        ...
        if (String(displayLineTotal.toFixed(2)) !== currentFormLineTotal) {
          setValue(`lineItems.${index}.total`, displayLineTotal.toFixed(2), { shouldValidate: false });
        }
      });
    }
    ...
    try {
      const newOrderId = await runTransaction(firestore, async (transaction) => {
        ...
        const processedLineItems = data.lineItems.map(item => {
            ...
            let itemName = 'N/A';
            if (itemDetailsFromOptions?.label) {
                const labelParts = itemDetailsFromOptions.label.split(' (');
                itemName = labelParts[0] || 'N/A';
            }
            ...
            const lineItemData: Record<string, any> = {
                ...
                total: parseFloat(itemTotalBeforeDiscount.toFixed(2)),
            };
    ...
    return (
      <Form {...form}>
        <form className="space-y-8">
          ...
          <Button type="button" variant="outline" onClick={() => append({ itemId: '', itemCode: '', description: '', qty: '1', unitPrice: '0         <Button type="button" variant="outline" onClick={() => append({ itemId: '', itemCode: '', description: '', qty: '1', unitPrice: '0', discountPercentage: '0', taxPercentage: '0', total: '0.00' })} className="mt-2"><PlusCircle className="mr-2 h-4 w-4" /> Add Item</Button>
