...
export function EditEmployeeForm({ employee }: EditEmployeeFormProps) {
  ...
  const [externalUrl, setExternalUrl] = React.useState(employee.photoURL || '');
    ...
    defaultValues: {
      ...
      salaryStructure: employee.salaryStructure ? {
        ...
        salaryBreakup: employee.salaryStructure.salaryBreakup?.map(sb => ({
          ...sb,
          amount: sb.amount || 0,
          increaseAmount: sb.increaseAmount || 0,
        })) || []
      } : {
        isConsolidate: false,
        paymentType: 'Bank',
  ...
  async function onSubmit(data: EmployeeFormValues) {
    ...
    try {
      ...
      // Handle policy history updates
      let updatedHistory = [...(employee.policyHistory || [])];
      const hasPolicyChanged = data.attendancePolicyId !== employee.attendancePolicyId;
      const isHistoryEmpty = updatedHistory.length === 0;

      if (data.attendancePolicyId && (hasPolicyChanged || isHistoryEmpty)) {
        // If the policy changed OR history is empty, we add/initialize the current assignment
        // Check if the last assignment is already the same to avoid duplicates
        const lastAssignment = updatedHistory[updatedHistory.length - 1];
        if (!lastAssignment || lastAssignment.policyId !== data.attendancePolicyId) {
          updatedHistory.push({
      ...
      const rawDataToSave = {
        ...
        attendancePolicyName: selectedAttendancePolicy?.name || employee.attendancePolicyName || '',
      ...
      delete (rawDataToSave as any).sameAsPresentAddress;

      // Recursively clean undefined values
      const cleanData = removeUndefined(rawDataToSave);

      // Add serverTimestamp after cleaning (cleanData is a new object)
      const finalDataToSave = {
        ...cleanData,
        updatedAt: serverTimestamp(),
      };

      await updateDoc(doc(firestore, "employees", employee.id), finalDataToSave);
  ...
  return (
    <Form {...form}>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex-1 space-y-6">
            <FormItem>
              ...
              <FormDescription>Upload a clear photo of the employee or toggle the link icon to provide an external URL.</FormDescription>
              <FormMessage />
            </FormItem>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <FormField control={control} name="maritalStatus" render={({ field }) => (<FormItem><FormLabel>Marital Status</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select status" /></SelectTrigger></FormControl><SelectContent>{maritalStatusOptions.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>)} />
