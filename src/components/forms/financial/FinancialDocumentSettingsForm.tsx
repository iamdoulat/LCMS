...
const financialSettingsSchema = z.object({
  ...
  logoWidth: z.number().min(16).max(1024).optional().default(396),
  logoHeight: z.number().min(16).max(512).optional().default(58),
  piHeaderTitle: z.string().optional(),
});
...
export function FinancialDocumentSettingsForm() {
      ...
      try {
        ...
        if (profileDocSnap.exists()) {
          ...
          form.reset({
            ...
            logoWidth: data.logoWidth || 396,
            logoHeight: data.logoHeight || 58,
            piHeaderTitle: data.piHeaderTitle || '',
          });
  ...
  function onImageLoad(e: React.SyntheticEvent<HTMLImageElement>) {
    ...
    const logoWidth = form.getValues('logoWidth') || 396;
    const logoHeight = form.getValues('logoHeight') || 58;
    ...
    <Form {...form}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Local PI Header Title</FormLabel>
                  <FormControl>
                    <Input placeholder="Local PI Header Title" {...field} value={field.value || ""} disabled={isReadOnly} />
                  </FormControl>
                  <FormDescription>This title will appear in the invoice header.</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
          ...
          <div className="space-y-6">
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        ...
                        placeholder="396"
                        {...field}
                        onChange={(e) => field.onChange(parseInt(e.target.value) || 396)}
                        value={field.value || 396}
                        disabled={isReadOnly}
                      />
                    </FormControl>
                    <FormMessage />
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        ...
                        placeholder="58"
                        {...field}
                        onChange={(e) => field.onChange(parseInt(e.target.value) || 58)}
                        value={field.value || 58}
                        disabled={isReadOnly}
                      />
                    </FormControl>
                    <FormMessage />
            ...
            <FormItem>
              ...
              <div className="flex items-start gap-4">
                ...
                >
                  {invoiceLogoUrl ? <Image src={invoiceLogoUrl} alt="Invoice Logo" width={form.watch('logoWidth') || 396} height={form.watch('logoHeight') || 58} className="object-contain w-full h-full" data-ai-hint="invoice logo" /> : <ImageIcon className="h-8 w-8 text-muted-foreground" />}
                </div>
                <div className="flex-1 space-y-3">
                  <Input type="file" accept="image/png, image/jpeg" onChange={onFileSelect} className="w-full" disabled={isReadOnly} />
                </div>
              </div>
              <FormDescription>Specific logo and title for quotes, invoices, etc. Upload a {form.watch('logoWidth') || 396}x{form.watch('logoHeight') || 58} pixels logo.</FormDescription>
            </FormItem>
          </div>
        </div>
      ...
      <Dialog open={isInvoiceLogoCropping} onOpenChange={setIsInvoiceLogoCropping}>
        <DialogContent className="max-w-xl">
          ...
          {invoiceLogoSrc && (
            <ReactCrop crop={invoiceLogoCrop} onChange={(_, c) => setInvoiceLogoCrop(c)} onComplete={(c) => setInvoiceLogoCompletedCrop(c)} aspect={(form.watch('logoWidth') || 396) / (form.watch('logoHeight') || 58)}>
