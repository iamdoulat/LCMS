...
import { HotspotSchema, type HotspotFormValues, type BranchDocument, type HotspotDocument } from '@/types'
...
interface EditHotspotFormProps {
    initialData: HotspotDocument
    onSuccess?: () => void
}

export function EditHotspotForm({ initialData, onSuccess }: EditHotspotFormProps) {
    ...
    const form = useForm<HotspotFormValues>({
        ...
        defaultValues: {
            ...
            allowRadius: initialData.allowRadius,
    ...
    // Watch for external updates to initialData (though typically this component will mount with data ready)
    useEffect(() => {
        form.reset({
            name: initialData.name,
            branchId: initialData.branchId,
    ...
    const onSubmit = async (values: HotspotFormValues) => {
        ...
        try {
            ...
            const hotspotRef = doc(firestore, 'hotspots', initialData.id)
            ...
            toast({ title: "Success", description: "Hotspot updated successfully." })
            router.push('/dashboard/hr/settings/hotspots')
            if (onSuccess) onSuccess()
        } catch (error: any) {
            console.error("Error updating hotspot:", error)
            toast({ title: "Error", description: error.message || "Failed to update hotspot.", variant: "destructive" })
        } finally {
