...
import type { OrderDocument, OrderFormValues, SupplierDocument, ItemDocument as ItemDoc } from '@/types';
...
export function EditPurchaseOrderForm({ initialData, orderId }: EditPurchaseOrderFormProps) {
  ...
  async function onSubmit(data: OrderFormValues) {
    ...
    try {
      const orderDocRef = doc(firestore, "purchase_orders", orderId); // UPDATED to purchase_orders
...
export default function EditNewPurchaseOrderPage() {
  return (
    <div className="container mx-auto py-8">
      {/* Wrapper handled inside EditPurchaseOrderForm */}
    </div>
  );
}
