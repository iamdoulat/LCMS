...
import { collection, doc, getDocs, updateDoc, serverTimestamp, Timestamp, query, orderBy } from 'firebase/firestore';
...
// Local Schema Definition (Same as Create form)
...
interface EditProjectInvoiceFormProps {
    initialData: ProjectInvoiceDocument;
    invoiceId: string;
}

export function EditProjectInvoiceForm({ initialData, invoiceId }: EditProjectInvoiceFormProps) {
    ...
    const form = useForm<ProjectInvoiceFormValues>({
        ...
        defaultValues: {
            ...
            packingCharge: initialData.packingCharge ? String(initialData.packingCharge) : undefined,
            handlingCharge: initialData.handlingCharge ? String(initialData.handlingCharge) : undefined,
            otherCharges: initialData.otherCharges ? String(initialData.otherCharges) : undefined,
        },
    });
    ...
    const handleCustomerChange = (value: string) => {
        ...
        const selectedCustomer = customerOptions.find(opt => opt.value === value);
        if (selectedCustomer) {
    ...
    async function onSubmit(data: ProjectInvoiceFormValues) {
        ...
        try {
            ...
            const dataToUpdate: Record<string, any> = {
                ...
                customerName: selectedCustomer ? selectedCustomer.label : initialData.customerName, // Keep existing if not found/changed
                projectId: data.projectId, projectTitle: selectedProject?.projectTitle || initialData.projectTitle || 'Unknown Project',
                taskId: data.taskId,
                ...
                totalAmount: grandTotal, status: data.status,
            ...
            // Refine taskTitle logic
            if (selectedTask) {
                dataToUpdate.taskTitle = selectedTask.taskTitle;
            } else if (!data.taskId) {
                // Explicitly set to null if taskId is cleared, so it removes the old title
                dataToUpdate.taskTitle = null;
            }
            // If taskId exists but task not found (unlikely), keep existing or just save ID. 
            // With strict typing, better to not overwrite if not sure, but 'null' is safe for removal.

            const cleanedDataToUpdate = Object.fromEntries(
                Object.entries(dataToUpdate).filter(([, value]) => value !== undefined)
            );

            const invoiceRef = doc(firestore, "project_invoices", invoiceId);
            await updateDoc(invoiceRef, cleanedDataToUpdate);

            Swal.fire({
                title: "Project Invoice Updated!",
                text: `Invoice ${invoiceId} successfully updated.`,
                icon: "success",
            });
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <Combobox
                                        ...
                                        onValueChange={(value) => handleCustomerChange(value === PLACEHOLDER_CUSTOMER_VALUE ? '' : value)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                    <FormItem>
                        ...
                        <Input value={invoiceId} readOnly disabled className="bg-muted/50 cursor-not-allowed h-10" />
                    </FormItem>
                    <FormField
                        control={control}
                        name="invoiceDate"
                ...
                <div className="flex flex-wrap gap-2 justify-end">
                    ...
                    <Button type="submit" className="bg-primary hover:bg-primary/90 text-primary-foreground" disabled={isSubmitting || isLoadingDropdowns}>
                        {isSubmitting ? (
                            <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Updating Invoice...</>
                        ) : (
                            <><Save className="mr-2 h-4 w-4" />Update Project Invoice</>
                        )}
                    </Button>
                </div>
            </form>
