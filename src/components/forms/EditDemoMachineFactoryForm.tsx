...
import type { DemoMachineFactory, DemoMachineFactoryDocument } from '@/types';
...
interface EditDemoMachineFactoryFormProps {
  initialData: DemoMachineFactoryDocument;
  factoryId: string;
}

export function EditDemoMachineFactoryForm({ initialData, factoryId }: EditDemoMachineFactoryFormProps) {
  ...
  async function onSubmit(data: DemoMachineFactoryFormValues) {
    ...
    try {
      ...
      await updateDoc(factoryDocRef, dataToUpdate);
      Swal.fire({
      ...
      console.error("Error updating demo machine factory document: ", error);
      const errorMessage = error instanceof Error ? error.message : "An unknown error occurred.";
      Swal.fire({
        title: "Update Failed",
        text: `Failed to update factory profile: ${errorMessage}`,
        icon: "error",
      });
    } finally {
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Enter incharge person's name" {...field} value={field.value ?? ''} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
              ...
              <FormItem>
                ...
                <div className="flex items-center gap-2">
                  <FormControl className="flex-grow">
                    <Input type="tel" placeholder="e.g., +8801XXXXXXXXX" {...field} value={field.value ?? ''} />
                  </FormControl>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea placeholder="Enter any additional notes for this factory" {...field} rows={4} value={field.value ?? ''} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
