...
export function EditPaymentTrackingEntryForm({ initialData }: EditPaymentTrackingEntryFormProps) {
  ...
  const [isLoadingLcDetails, setIsLoadingLcDetails] = React.useState(true);
  ...
  React.useEffect(() => {
    const fetchLcDetails = async () => {
      if (initialData.id) {
        setIsLoadingLcDetails(true);
        try {
            const lcDocRef = doc(firestore, "lc_entries", initialData.id);
            const lcDocSnap = await getDoc(lcDocRef);
            if (lcDocSnap.exists()) {
                const lcData = lcDocSnap.data() as LCEntryDocument;
                setSelectedLcDetails(lcData);
                // Also update the lcOptions so the combobox can find the label
                setLcOptions([{ value: lcData.id, label: lcData.documentaryCreditNumber || 'Unnamed L/C' }]);
            } else {
                 console.error("Could not find the linked L/C document with ID:", initialData.id);
                 setSelectedLcDetails({ documentaryCreditNumber: "L/C Not Found" });
            }
        } catch(e) {
            console.error("Error fetching LC details:", e);
        } finally {
            setIsLoadingLcDetails(false);
        }
      }
    };
    fetchLcDetails();
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Combobox
                  ...
                  selectPlaceholder={isLoadingLcDetails ? "Loading..." : "Select L/C"}
                  disabled={true}
                />
                <FormMessage />
              </FormItem>
          ...
          {selectedLcDetails ? (
            <div className="grid grid-cols-2 gap-4 text-sm p-4 border rounded-md bg-muted/50">
             ...
             <div className="p-4 border rounded-md bg-muted/50 text-muted-foreground text-sm">
                {isLoadingLcDetails ? <div className="flex items-center"><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Loading L/C details...</div> : "Could not load L/C details."}
             </div>
          )}
        </div>
        <Separator/>
