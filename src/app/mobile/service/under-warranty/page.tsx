...
export default function MobileUnderWarrantyPage() {
    const router = useRouter();
    const [machines, setMachines] = useState<UnderWarrantyMachine[]>([]);
    ...
    const fetchUnderWarrantyData = useCallback(async (isNextPage = false) => {
        ...
        try {
            ...
            let gatheredMachines: UnderWarrantyMachine[] = [];
    ...
    const handleDownloadPdf = (machine: UnderWarrantyMachine) => {
        const doc = new jsPDF();
        ...
        autoTable(doc, {
            ...
            body: [
                ...
                ['Status', `${machine.daysRemaining} Days Remaining`],
            ],
            theme: 'striped',
        ...
        doc.save(`Warranty_Report_${machine.serialNo}.pdf`);
    };
                    ...
                    <Sheet open={isFilterOpen} onOpenChange={setIsFilterOpen}>
                        ...
                        <SheetContent side="bottom" className="rounded-t-[2.5rem] p-8 max-h-[85vh] overflow-y-auto border-none">
                            <SheetHeader className="mb-6 text-left">
                                ...
                                <p className="text-sm text-slate-500">Find specific machines under warranty.</p>
                            </SheetHeader>
                            ...
                            <div className="space-y-4 pb-8">
                                ...
                                <Button
                                    className="w-full h-14 rounded-[1.5rem] bg-[#0a1e60] hover:bg-blue-900 text-white font-bold text-lg mt-4 shadow-lg active:scale-[0.98] transition-all"
                ...
                {isLoading ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-4">
                        <div className="relative">
                            ...
                            <ShieldCheck className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-6 w-6 text-[#0a1e60]" />
                        </div>
                        <p className="text-slate-500 font-bold animate-pulse">Scanning warranties...</p>
                    </div>
                    ...
                    <div className="space-y-4">
                        {filteredMachines.map((machine, index) => (
                            <div
                                ...
                                className="bg-white p-5 rounded-[2.5rem] shadow-[0_20px_40px_-12px_rgba(0,0,0,0.1)] border-2 border-[#0a1e60]/5 hover:border-[#0a1e60]/20 active:scale-[0.98] transition-all group relative overflow-hidden"
                            >
                                {/* Side Accent */}
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex items-center gap-3">
                                        <div className="h-12 w-12 bg-blue-50 rounded-2xl flex items-center justify-center border border-blue-100 shadow-sm group-hover:scale-110 transition-transform">
                                    ...
                                    <div className={cn(
                                        "px-3 py-1.5 rounded-xl border flex items-center gap-2 shadow-sm font-black text-[10px]",
                                        machine.daysRemaining <= 30 ? "bg-rose-50 border-rose-100 text-rose-600" :
                                            machine.daysRemaining <= 90 ? "bg-amber-50 border-amber-100 text-amber-600" :
                                                "bg-emerald-50 border-emerald-100 text-emerald-600"
                                    )}>
                                        <ShieldCheck className="h-3 w-3" />
                                        <span>{machine.daysRemaining} Days</span>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center justify-between gap-4 pt-4 border-t border-dashed border-slate-100">
                                    <div className="flex flex-col">
                                        <span className="text-[9px] font-bold text-slate-400 uppercase">Expiry Date</span>
                                        <span className="text-[11px] font-black text-slate-700">{format(parseISO(machine.warrantyExpiryDate), 'MMM d, yyyy')}</span>
                                    </div>
                                    <Button
                                        className="h-11 rounded-xl bg-[#0a1e60] hover:bg-blue-900 text-white font-black text-xs gap-2 px-6 shadow-md active:scale-95 transition-all group"
                        ...
                        {hasMore && (
                            <div className="mt-8 mb-4 flex justify-center">
                                <Button
                                    onClick={() => fetchUnderWarrantyData(true)}
