...
export default function MobileAddErrorCodePage() {
    ...
    useEffect(() => {
        if (userRole && !userRole.includes('Admin') && !userRole.includes('Service') && !userRole.includes('Super Admin') && !userRole.includes('Supervisor')) {
            Swal.fire("Access Denied", "You do not have permission to add error codes.", "error");
        ...
        <div className="flex flex-col h-full bg-[#0a1e60] overflow-hidden">
            ...
            <div className="sticky top-0 z-50 px-4 pt-4 pb-6 bg-[#0a1e60]">
                ...
                <div className="relative flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-white tracking-tight">Add Error Code</h1>
                        <p className="text-[10px] font-bold text-blue-400/80 uppercase tracking-[0.2em]">New Guide</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 bg-white rounded-t-[2.5rem] shadow-inner p-6 pb-32 overflow-y-auto">
                <div className="max-w-md mx-auto">
                    <div className="mb-8 flex items-center gap-4 p-4 bg-rose-50 rounded-3xl border border-rose-100/50">
                        <div className="h-12 w-12 rounded-2xl bg-white flex items-center justify-center text-rose-600 shadow-sm">
                            <FileCode className="h-6 w-6" />
                        </div>
                        <div>
                            <p className="text-xs font-bold text-rose-600/60 uppercase tracking-wider">Troubleshooting</p>
                            <p className="text-[11px] font-medium text-slate-500 leading-tight">Add a new machine error code and its technical solution.</p>
                        </div>
                    </div>
