...
export default function EditPurchaseOrderPage() {
  ...
  const fetchOrderData = useCallback(async () => {
    if (!orderId) {
      ...
      Swal.fire("Error", "No Purchase Order ID specified.", "error").then(() => {
        router.push('/dashboard/purchase-orders/list');
      });
      return;
    }
    setIsLoading(true);
    ...
    try {
      ...
      Swal.fire("Error", `Failed to fetch purchase order data: ${err.message}`, "error");
    } finally {
      setIsLoading(false);
    }
  }, [orderId, router]);
  ...
  if (isLoading) {
    return (
      <div className="container mx-auto py-8 flex flex-col items-center justify-center min-h-[calc(100vh-10rem)]">
        ...
        <p className="text-muted-foreground">Loading purchase order details for ID: {orderId}...</p>
      </div>
    );
  }
      ...
      <div className="container mx-auto py-8">
        <Card className="max-w-screen-2xl mx-auto shadow-xl border-destructive">
          ...
          <CardContent><p className="text-destructive-foreground">{error}</p>
            <Button variant="outline" asChild className="mt-4"><Link href="/dashboard/purchase-orders/list"><ArrowLeft className="mr-2 h-4 w-4" />Back to Purchase Orders List</Link></Button>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="container mx-auto py-8">
      <div className="mb-6"><Link href="/dashboard/purchase-orders/list" passHref><Button variant="outline"><ArrowLeft className="mr-2 h-4 w-4" />Back to Purchase Orders List</Button></Link></div>
      <Card className="max-w-screen-2xl mx-auto shadow-xl">
        <CardHeader>
          <CardTitle className={cn("flex items-center gap-2", "font-bold text-2xl lg:text-3xl bg-gradient-to-r from-[hsl(var(--primary))] via-[hsl(var(--accent))] to-rose-500 text-transparent bg-clip-text hover:tracking-wider transition-all duration-300 ease-in-out")}>
            <ShoppingCart className="h-7 w-7 text-primary" />Edit Purchase Order
          </CardTitle>
