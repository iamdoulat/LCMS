...
export default function EditPaymentTrackingPage() {
  const params = useParams();

  const id = params.id as string;

  const [trackerData, setTrackerData] = React.useState<DeferredPaymentRecord | null>(null);
  ...
  React.useEffect(() => {
    ...
    const fetchTrackerData = async () => {
      ...
      setError(null);
      try {
        const docRef = doc(firestore, "deferred_payment_tracker", id);
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          setTrackerData({ id: docSnap.id, ...docSnap.data() } as DeferredPaymentRecord);
        } else {
          setError("Deferred payment tracking record not found.");
        ...
        <Card className="max-w-5xl mx-auto shadow-xl border-destructive">
          ...
          <CardContent>
            ...
            <Button variant="outline" asChild className="mt-4">
              <Link href="/dashboard/deferred-payment-tracker">
      ...
      <Card className="max-w-5xl mx-auto shadow-xl">
        <CardHeader>
          <CardTitle className={cn("font-bold text-2xl lg:text-3xl flex items-center gap-2", "bg-gradient-to-r from-[hsl(var(--primary))] via-[hsl(var(--accent))] to-rose-500 text-transparent bg-clip-text hover:tracking-wider transition-all duration-300 ease-in-out")}>
            ...
            Edit Payment Tracking Entry
          </CardTitle>
          <CardDescription>
            Modify the details for entry linked to L/C: <span className="font-semibold text-foreground">{trackerData.documentaryCreditNumber}</span>
          </CardDescription>
        </CardHeader>
        <CardContent>
          <EditPaymentTrackingEntryForm initialData={trackerData} />
        </CardContent>
      </Card>
    </div>
  );
