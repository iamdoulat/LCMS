...
import { Loader2, PlusCircle, Trash2, Users, Building, FileText, CalendarDays, User, DollarSign, Hash, Percent, Ship, Save, Link2, MinusCircle, ExternalLink, Link as LinkIcon, ListChecks, TrendingUp } from 'lucide-react';
...
type CommissionInvoiceFormValues = z.infer<typeof commissionInvoiceSchema>;

interface EditCommissionInvoiceFormProps {
...
export function EditCommissionInvoiceForm({ initialData, piId }: EditCommissionInvoiceFormProps) {
    ...
    const [grandTotalSalesWithOvi, setGrandTotalSalesWithOvi] = React.useState(0);
    ...
    const [totalOVI, setTotalOVI] = React.useState(0);
    const [grandTotalCommissionWithOvi, setGrandTotalCommissionWithOvi] = React.useState(0);
    ...
    const form = useForm<CommissionInvoiceFormValues>({
        ...
        defaultValues: {
            ...
            lineItems: initialData?.lineItems?.map(item => ({
                ...
                oviAmount: item.oviAmount?.toString() || '',
                netCommissionPercentage: item.netCommissionPercentage?.toString() || '',
            })) || [{ slNo: '1', modelNo: '', qty: '', purchasePrice: '', salesPrice: '', oviAmount: '', netCommissionPercentage: '' }],
    ...
    }, [initialData, form, isLoadingDropdowns, lcOptions]);
    ...
    React.useEffect(() => {
        ...
        const finalTotalOVI = newTotalSalesWithOVI - newTotalSalesLineItems;
        setTotalOVI(finalTotalOVI);
        ...
        setGrandTotalSalesWithOvi(currentGrandTotalSalesPrice + finalTotalOVI);
    ...
    async function onSubmit(data: CommissionInvoiceFormValues) {
        ...
        let calculatedTotalSalesWithOVI = 0;
        ...
        const processedLineItems = data.lineItems.map(item => {
            ...
            const ovi = parseFloat(String(item.oviAmount || '0'));
            ...
            if (ovi > 0) lineItemData.oviAmount = ovi;
            if (netCommP > 0) lineItemData.netCommissionPercentage = netCommP;

            return lineItemData;
        });

        const calculatedTotalOVI = calculatedTotalSalesWithOVI - calculatedTotalSalesPriceLineItems;
        ...
        const dataToUpdate: Partial<Omit<ProformaInvoiceDocument, 'id' | 'createdAt'>> = {
            ...
            totalOVI: calculatedTotalOVI > 0 ? calculatedTotalOVI : undefined,
            grandTotalSalesPrice: finalGrandTotalSalesPrice,
            grandTotalSalesWithOvi: finalGrandTotalSalesPrice + calculatedTotalOVI,
        ...
        try {
            ...
            Swal.fire({
                title: "Commission Invoice Updated!",
                text: `Commission Invoice "${data.piNo}" has been successfully updated.`,
                icon: "success",
                timer: 1000,
            ...
            Swal.fire({
                title: "Update Failed",
                text: `Failed to update Commission Invoice: ${error.message}`,
                icon: "error",
            });
        } finally {
    ...
    const handleAddLineItem = () => {
        append({ slNo: (fields.length + 1).toString(), modelNo: '', qty: '', purchasePrice: '', salesPrice: '', oviAmount: '', netCommissionPercentage: '' });
    };
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                <FormLabel className="flex items-center"><FileText className="mr-2 h-4 w-4 text-muted-foreground" />Invoice No.*</FormLabel>
                                <FormControl>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
                    ...
                    <div className="space-y-3 p-5 border rounded-xl shadow-sm bg-background/50 backdrop-blur-sm border-blue-100 dark:border-blue-900/30">
                        <h5 className="font-bold text-blue-600 flex items-center gap-2 mb-4 border-b pb-2">
                            <Hash className="h-4 w-4" /> General Statistics
                        </h5>
                        <div className="space-y-2 text-sm">
                            <div className="flex justify-between items-center">
                                <span className="text-muted-foreground font-medium">Total Qty:</span>
                                <span className="font-bold text-foreground text-lg">{totalQty}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-muted-foreground font-medium">Total Sales (Items):</span>
                                <span className="font-bold text-foreground">{totalSalesPriceFromLineItems.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-muted-foreground font-medium">Extra Net Comm.:</span>
                    ...
                    {/* Column 2: Sales Totals */}
                    <div className="space-y-3 p-5 border rounded-xl shadow-sm bg-background/50 backdrop-blur-sm border-emerald-100 dark:border-emerald-900/30">
                        <h5 className="font-bold text-emerald-600 flex items-center gap-2 mb-4 border-b pb-2">
                            <DollarSign className="h-4 w-4" /> Sales & Performance
                        </h5>
                        <div className="space-y-2 text-sm">
                            <div className="flex justify-between items-center">
                                <span className="text-muted-foreground font-medium">Total Purchase Price:</span>
                                <span className="font-bold text-foreground">{totalPurchasePriceAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                            </div>
                            <div className="flex flex-col mt-4 p-3 bg-emerald-50 dark:bg-emerald-950/20 rounded-lg">
                                <span className="text-emerald-700 dark:text-emerald-400 text-[10px] font-bold uppercase tracking-wider">Grand Total Sales (Comm.)</span>
                                <span className="text-2xl font-black text-emerald-600 tracking-tight">${grandTotalSalesPrice.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                            </div>
                            <div className="flex flex-col mt-2 p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg">
                                <span className="text-blue-700 dark:text-blue-400 text-[10px] font-bold uppercase tracking-wider">Grand Total Comm. USD</span>
                                <span className="text-2xl font-black text-blue-600 tracking-tight">${grandTotalCommissionUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                            </div>
                        </div>
                    </div>
