...
interface PaymentPendingLC extends LCEntryDocument {
  updatedAtDate: Date; // Keep this if you use it for display
}

const ITEMS_PER_PAGE = 10;
const ALL_YEARS_VALUE = "__ALL_YEARS_PAYMENT_PENDING__";
const PLACEHOLDER_APPLICANT_VALUE = "__ALL_APPLICANTS_PAYMENT_PENDING__";
...
export default function LCPaymentPendingPage() {
  ...
  useEffect(() => {
    const fetchPaymentPendingLCs = async () => {
      setIsLoading(true);
      setFetchError(null);
      try {
    ...
    const fetchFilterOptions = async () => {
      ...
      try {
        ...
        console.error("Error fetching filter options for L/C Payment Pending page:", error);
    ...
    fetchPaymentPendingLCs();
    fetchFilterOptions();
  }, []);

  useEffect(() => {
    let filtered = [...allPaymentPendingLCs];
    ...
    setDisplayedPaymentPendingLCs(filtered);
    setCurrentPage(1);
  }, [allPaymentPendingLCs, filterLcNumber, filterApplicantId, filterBeneficiaryId, filterYear]);
  ...
  const totalPages = Math.ceil(displayedPaymentPendingLCs.length / ITEMS_PER_PAGE);
  const indexOfLastItem = currentPage * ITEMS_PER_PAGE;
  const indexOfFirstItem = indexOfLastItem - ITEMS_PER_PAGE;
  const currentItems = displayedPaymentPendingLCs.slice(indexOfFirstItem, indexOfLastItem);
      ...
      <Card className="shadow-xl">
        <CardHeader>
          <CardTitle className={cn("font-bold text-2xl lg:text-3xl flex items-center gap-2 text-primary", "bg-gradient-to-r from-[hsl(var(--primary))] via-[hsl(var(--accent))] to-rose-500 text-transparent bg-clip-text hover:tracking-wider transition-all duration-300 ease-in-out")}>
            ...
            L/Cs with Payment Pending
          </CardTitle>
          <CardDescription>
            List of Letters of Credit marked as "Payment Pending", sorted by most recent update.
            Showing {currentItems.length > 0 ? indexOfFirstItem + 1 : 0}-{Math.min(indexOfLastItem, displayedPaymentPendingLCs.length)} of {displayedPaymentPendingLCs.length} entries.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Card className="mb-6 shadow-md p-4">
            ...
            <CardContent className="p-2 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                <div>
                  <Label htmlFor="lcNoFilterPaymentPending" className="text-sm font-medium">L/C Number</Label>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 border-2 border-dashed border-muted-foreground/30 rounded-lg bg-muted/20 p-6">
              ...
              <p className="text-sm text-muted-foreground text-center">
                There are no L/Cs currently marked as "Payment Pending" matching your criteria, or the required Firestore index is missing/still building.
              </p>
            </div>
          ) : (
            ...
            <div className="flex items-center justify-center space-x-2 py-4 mt-4">
              ...
              {getPageNumbers().map((page, index) =>
                typeof page === 'number' ? (
                  ...
                  <span key={`ellipsis-payment-pending-${index}`} className="px-2 py-1 text-sm">
                    {page}
                  </span>
                )
              )}
