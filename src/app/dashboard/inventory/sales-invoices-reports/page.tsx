...
import { BarChart3, Download, FileText, Loader2, Filter, XCircle } from 'lucide-react';
...
export default function SalesInvoicesReportsPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                // Fetch Invoices
                const invoicesQuery = query(collection(firestore, "sales_invoice"), orderBy("createdAt", "desc"));
    ...
    }, [allInvoices, dateRange, filterCustomerId, filterStatus, filterInvoiceNo]);
    ...
    // Assuming 'Paid' status means fully paid. Partial logic might need 'receivedAmount' if available, defaulting to total for 'Paid'
    // For this report, let's keep it simple: Paid invoices count towards Total Paid. 
    // If you have partial payments, you would sum those up. Since I don't see 'receivedAmount' property in my quick view, 
    // I'll assume status 'Paid' = totalAmount paid.
    const totalPaid = displayedInvoices
        .filter(inv => inv.status === 'Paid')
        .reduce((sum, inv) => sum + (inv.totalAmount || 0), 0);

    // Total Due
    // If status is Unpaid, Overdue, Partial (assuming full amount due for simplicity or just substract paid)
    // To be accurate without a 'dueAmount' field, we can just show Total Sales and Total Paid (based on Paid status).
    // Or if there is a 'paidAmount' field on invoice, we use that. 
    // Let's stick to Total Generated Value for now.
    ...
    const downloadPDF = () => {
        ...
        doc.save(`sales_invoices_report_${format(new Date(), 'yyyy-MM-dd')}.pdf`);
    };

    const downloadCSV = () => {
        const headers = ["Invoice No,Date,Customer,Status,Amount"];
        ...
        link.setAttribute("download", `sales_invoices_report_${format(new Date(), 'yyyy-MM-dd')}.csv`);
        ...
        <div className="w-full p-4 md:p-6 space-y-6">
            <Card className="shadow-lg border-l-4 border-l-primary">
                <CardHeader>
                    ...
                    <CardDescription>
                        Generate and download customized reports for sales invoices.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    {/* Filters */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        ...
                        <Card className="bg-slate-50 dark:bg-slate-900 border-none shadow-sm">
                            <CardContent className="p-4 flex flex-col items-center justify-center text-center">
                                ...
                                <p className="text-2xl font-bold text-green-600">BDT {totalSales.toLocaleString()}</p>
                            </CardContent>
                        </Card>
                        <Card className="bg-slate-50 dark:bg-slate-900 border-none shadow-sm">
                            <CardContent className="p-4 flex flex-col items-center justify-center text-center">
                                <p className="text-sm font-medium text-muted-foreground">Active Filter</p>
                                <p className="text-sm font-bold text-blue-600 truncate max-w-full px-2">
                                    {dateRange?.from ? 'Date Range Applied' : 'All Dates'} • {filterStatus !== 'All' ? filterStatus : 'All Status'}
                                </p>
                            </CardContent>
                        </Card>
                    </div>
