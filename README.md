...
*   **T/T OR L/C Management**: Your central hub for all trade finance activities.
    ...
    *   **LC Expire Tracker**: A dedicated module to monitor and manage L/C expiration dates. Proactively identify L/Cs that require amendment or renewal with visual alerts.
...
*   **Quotes and Invoices**: Manage your entire sales documentation process from start to finish.
    ...
    *   **Layout Settings & Custom Dimensions**: Customize the look and feel of your printable documents. Set exact pixel dimensions for logos and header titles for perfect print alignment.
...
*   **Accounts and Inventory**: Gain full control over your stock and sales records.
    ...
    *   **Premium Item Management**: Maintain a complete inventory with a newly designed, premium interface.
        *   **Standardized Workflows**: Consistent, intuitive forms for Adding and Editing items.
        *   **Thematically Grouped Sections**: Pricing (Green theme) and Inventory (Blue theme) sections for better visual focus.
        *   **Sticky Visual Previews**: Keep product images in view even during long form edits.
        *   **Legacy Data Support**: Gracefully handles manual category/section entries with clear labeling.
    ...
    *   **Settings**: Configure petty cash accounts, transaction categories, and fully manage Inventory definitions (Item Categories, Variations, and Sections) from a central settings page.
...
*   **Shipment Management**: Never lose sight of your goods with real-time tracking and status updates.
    ...
    *   **Deferred Payment Tracker**: A robust module to monitor deferred L/Cs with advanced capabilities:
        *   **Value Tracking**: Monitor both L/C Value and specific Shipment Values.
        *   **Partial Shipment Support**: Track and distinguish between 1st, 2nd, and 3rd shipments within a single L/C.
        *   **Maturity Automation**: Automatically calculate and display payment maturity dates based on shipment data.
...
Complete workforce management solution designed for both office and field employees.
 
*   **Daily Attendance System**: A robust system to track and manage daily employee attendance.
    *   **Comprehensive Tracking**: Log attendance with detailed statuses including Present, Absent, Late, Leave, Official Visit, Holiday, and Weekend.
...
*   **Supervisor & Team Features**:
    ...
    *   **Team History**: View recent check-in/out records only for team members under your supervision.
    *   **Management Modal**: Admins can easily assign or reassign supervisors to various employees.

*   **Field Force Management (Multiple Check-In/Out)**:
    *   **Remote Attendance**: Perfect for field agents to record client visits or remote work sessions.
    *   **Proof of Presence**: Requires mandatory **Geolocation** capture and **Real-time Photo** (camera only) to ensure authenticity.
    *   **Automated Checkout**: System-level safety that automatically checks out visits exceeding a defined hourly limit (e.g., 8 hours).
    *   **Visit Details**: Capture specific company names and remarks for every check-in/out event.
 
*   **Employee Self-Service**:
    *   **Personal Dashboard**: Employees can view their own monthly attendance summary (Present, Absent, Late, Leaves).
    *   **Profile Management**: Tools for employees to update their profile pictures with cropping functionality.
    *   **Geo-Fenced Clocking**: Allow employees to mark their own attendance with location validation directly from their portal.

### Notification & System Governance

*   **Intelligent Notification Engine**:
    *   **WhatsApp & Email Integration**: Automated alerts for account creation, leave requests, salary advances, and shipment updates.
    *   **Template Management**: Enable or disable specific notifications on the fly without changing code.
...
*   **Granular Roles**: Assign users specific roles like `Super Admin`, `Admin`, `Accounts`, `Commercial`, `Service`, `Supervisor`, `DemoManager`, or `Viewer`.
*   **Supervisor Data Isolation**: Supervisors can see attendance and history for their team only, while HR/Admins maintain global visibility.
...
*   **Built for Speed**: Powered by **Next.js** and **React**, our application is incredibly fast, with server-side rendering and optimized client-side navigation.
*   **Mobile-First Design**: Enhanced spacing and padding across all core dashboard pages ensures a premium mobile experience.
*   **Secure & Scalable Backend**: Powered by **Firebase (Firestore)** and **Authentication** for real-time data sync and military-grade security.
*   **Custom Design System**: A bespoke UI built with **Tailwind CSS** and **ShadCN** that avoids the "bootstrap look" in favor of a modern, professional aesthetic.
...
3.  **Register Account**: Create a secure account; Super Admins will assign your role via the Settings module.
4.  **Configure Templates**: Navigate to `Settings > Notification Templates` to enable/disable specific email and WhatsApp alerts.
5.  **Define Team Structure**: For HR/Admins, use the **User Management** section to assign supervisors to employees to enable team-based features.
6.  **Start Managing**: Create your first LC, add inventory items, or track field visits immediately.
